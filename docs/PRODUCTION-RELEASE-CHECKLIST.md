...
ถ้า Netlify ตั้ง auto deploy จาก `main` ให้รอ deploy ของ commit ล่าสุดสำเร็จและไม่สั่ง deploy ซ้ำ

ถ้าใช้ CLI deploy แบบควบคุมเอง:

```powershell
netlify deploy --prod --dir .
```

ก่อนใช้ CLI ต้องยืนยันว่า working tree ไม่มีไฟล์ที่ยังไม่ commit ซึ่งอาจทำให้ Production ไม่ตรงกับ GitHub
...
- [ ] Commit และ push สำเร็จ โดย Production ตรงกับ commit ที่ระบุ
...
13. รายงานปิดงานต้องสั้น ชัด และมี commit, deployment, test result, limitation และ rollback target
