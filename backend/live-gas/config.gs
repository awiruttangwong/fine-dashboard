var BACKEND_CONFIG = {
  ...
  // schema ชีตในคลังกลาง — ลูกค้าเป็นคอลัมน์สุดท้าย (M, index 12) ต่อท้าย ห้ามแทรกกลาง
  // เพราะโค้ดอ้างคอลัมน์ด้วยตำแหน่ง index (ตรงกับ DEBT_SCHEMA เดิมใน Code.merged)
  // ── 2026-08-06: ถอด 'ชื่อ พขร' (เดิมคอลัมน์ C) ออกจาก schema ตามที่เลิกใช้งานฟิลด์นี้
  // ทั้งระบบ (ทั้ง import, ฟอร์มเพิ่ม/แก้ไข และการแสดงผล) คอลัมน์หลังจากนั้นเลื่อนซ้าย 1
  // ช่องทั้งหมด → ชีตเก่าที่ยังมี 14 คอลัมน์ต้องรัน migrateDropDriverNameColumn ก่อน
  debtSchema: {
    drivers: {
      name: 'Drivers',
      headers: ['รหัส', 'ชื่อผู้รับโอน', 'เส้นทาง', 'วันที่เริ่ม', 'ยอดรวม', 'จำนวนงวด', 'ชำระแล้ว', 'คงเหลือ', 'สถานะ', 'สาเหตุ', 'ปรับได้/ปรับไม่ได้', 'ประวัติการย้าย', 'ลูกค้า']
    },
    payments: {
      name: 'Payments',
  ...
  monthlySources: [
    ...
    { label: 'M10', id: '1a7QQQiLZ6P5lsqhwouqtDxAev_JSl6MCFFpRTuMO-6U' },
    { label: 'M11', id: '' },
    { label: 'M12', id: '' }
  ],
